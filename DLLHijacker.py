...
def main():
    try:
        ...
        hijacker(dllname, folder, exportTable, is32bit, filepath)
...
def hijacker(dllname, gen_folder, symbols, is32bit, filepath):
    os.mkdir(gen_folder)
...
'''
    ...
    if is32bit:
        ...
        with open(sub_folder + '\\' + dllname + '.asm', "w", encoding="utf-8") as f:
            f.writelines(asm)
...
'''
    ...
    dllmain = dllmain.replace('TEAMPLATE_DLLNAME', filepath)
    dllmain = dllmain.replace('HIJACKFUNC', hijackfunc)

    with open(sub_folder + '\\' + 'dllmain.cpp', "w", encoding="utf-8") as f:
...
# source.def
    ...
    with open(sub_folder + r'\Source.def', "w", encoding="utf-8") as f:
...
'''
    ...
    with open(sub_folder + r'\pch.h', "w", encoding="utf-8") as f:
        f.writelines(txtPchHeader)

    txtPchCpp = '''# include "pch.h"'''
    with open(sub_folder + r'\pch.cpp', "w", encoding="utf-8") as f:
...
'''
    ...
    with open(sub_folder + r'\framework.h', "w", encoding="utf-8") as f:
...
''').replace('TEMPLATE_DLLNAME', dllname)
        ...
        with open(sub_folder + '\\' + dllname + '.vcxproj', "w", encoding="utf-8") as f:
            f.write(txtVcxproj)
...
''').replace('TEMPLATE_DLLNAME', dllname)
        ...
        with open(sub_folder + '\\' + dllname + '.vcxproj.filters', "w", encoding="utf-8") as f:
            f.write(txtFilter)
